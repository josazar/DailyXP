...
void main()
{
  ...
  p.x +=noise+abs(atan(u_time)*u_time*.5);
  vec3 col  = vec3(0.);
  for(int i = 0; i < 50; i++){
    vec2 t = vec2(p.x + float(i)*250., -p.y*cos(u_time)*5.*float(i)-float(i/2)) / min(u_resolution.x, u_resolution.y) * 3.0;
    t.xy += vec2(-u_resolution.x, u_resolution.y) / min(u_resolution.x, u_resolution.y);
    t.y +=cos(float(i))*.75;
     float v = 0.02 / length(p -t);
    vec3 c = vec3(smoothstep(0.01, .475, v));
    col += c;
  }
