...
float GetDist(vec3 p)
{
    ...
    vec3 pt = p - vec3(0.+(cos(u_time)*0.1),1,6.+(cos(u_time)*0.2));
    pt.zy *= Rot(PI*0.5);
    pt.xy *= Rot(PI*0.15);
    float torus = sdTorus(pt, vec2(.5,.1));
    pt.x +=cos(u_time)*0.2;
    pt.y +=cos(u_time)*0.2;
    float t2 = sdTorus(pt, vec2(.7,.1));
    pt.zy *= Rot(PI*0.05 * cos(u_time));
    pt.x +=cos(u_time)*0.1;
    pt.y +=cos(u_time)*0.1;
    float t1 = sdTorus(pt, vec2(.3,.1));
    pt.x +=cos(u_time)*0.2;
    pt.y +=cos(u_time)*0.2;
    float t3 = sdTorus(pt, vec2(.13,.05));
    ...
    float d = min(torus*0.6,planeDist);
    d = min(d, t2);
    d = min(d, t1);
    d = min(d, t3);
...
void main()
{
    ...
    vec3 ro = vec3(0.2,1,1.5); // Ray Origin/ Camera
