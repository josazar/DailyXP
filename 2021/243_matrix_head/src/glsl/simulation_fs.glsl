...
void main(){
  ...
  finalPos=mix(pos,curlPos,cnoise(pos+t) * .12);

  float offSet = .25;
  life = cos( finalPos.y * uv.x * t * 100.) * .5 + .5;

  // Mouse Pos
  if(distance(finalPos,uCursorPos) < .85) {
    // finalPos=mix(pos,curlPos,cnoise(pos+t));
    // finalPos = mix(finalPos, uCursorPos, - 0.12 );
    finalPos.y += life * 0.5;
    // finalPos.x = mix(pos,curlPos*5.,cnoise(pos+t) * .12).x;
    life = 1. - life;
    life *= 0.4;
  }
  ...
  gl_FragColor=vec4(finalPos,life);
}
