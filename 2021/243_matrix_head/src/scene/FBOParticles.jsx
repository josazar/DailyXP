...
const FBOParticles = ({ renderer }) => {
	...
	useEffect(() => {
		console.log(Math.floor(Math.sqrt(josephGeometry.attributes.position.array.length / 3, 2)))
		console.log(josephGeometry.attributes.position.array.length / 3)
		// width and height of FBO
		// const width = 256
		// const height = 256		
		
		const width = Math.floor(Math.sqrt(josephGeometry.attributes.position.array.length / 3, 2)) / 1.5
		const height = width
