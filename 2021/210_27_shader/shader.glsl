...
void main() {
	...
	float ease = easeBackInOut(cos(u_time)*.5+.85);
	float easeHeart = easeBounceOut(cos(u_time*2.)*.5+.85);

	color += fill(heartSDF(st), .1*easeHeart);
	color -= stroke(polySDF(st, 3),.1,.05);
	color += stroke(raysSDF(st, 4),.45*ease,.2);
