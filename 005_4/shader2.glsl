  ...
  float snoise(vec2 v){
    ...
    vec3 p=permute(
      ...
      void main(){
        ...
        // fraction
        //pos=fract(pos*1.);
        vec3 bleu=vec3(0.,1.,.7843);
        
        float n=snoise(pos+cos(abs(sin(u_time))*.5));
        
        float line=step(lines(pos+n,.5),.85);
        vec3 col=mix(vec3(.5,.02,.238),bleu,.5)+line/10.;
        
        // rect
        for(float i=0.;i<40.;i++){
          pos=rot(pos,u_time/50.);
          float l=.0002;
          float h=.32;
          float posX=cos(.5+u_time/2.)*.5+.5;
          float posY=.8;
          col=mix(col,vec3(0.),rect(pos+vec2(posX,posY),vec2(l,h)));
          col=mix(col,bleu,rect(pos+vec2(posX,posY),vec2(l-.00005,h-.005)));
