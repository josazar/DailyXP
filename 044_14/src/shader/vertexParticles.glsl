...
void main()
{
  ...
  vPosition.y+=cos(u_time+vIndex)*size/60.*(1.-phytoncide);
  // Second model position
  vPositionA.y+=cos(u_time+vIndex)*size/60.*(1.-phytoncide);
  // phytoncide
  // --------------------------------
  ...
  float phytoncideSize=(size*25.)*(1./-mvPosition.z);
  gl_PointSize=mix((4.+size*6.)*(1./-mvPosition.z),phytoncideSize,phytoncide);
