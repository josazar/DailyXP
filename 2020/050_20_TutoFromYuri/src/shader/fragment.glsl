...
void main(){
  ...
  vec4 txt=texture2D(skin,newFakeUV+.2*cnoise(vec4(newFakeUV*5.,time/20.,0.),vec4(6.)));
  gl_FragColor=vec4(vNormal,1.);
  // gl_FragColor=vec4(abs(sin(diff*50.)));
  // gl_FragColor=vec4(newFakeUV,0.,1.);0
  gl_FragColor=txt;
  gl_FragColor=vec4(fresnelTerm);
  gl_FragColor=vec4(mix(vec3(1.),txt.rgb,fresnelTerm),1.);
  // gl_FragColor=vec4(txt.rgb*fresnelTerm,1.);
}
