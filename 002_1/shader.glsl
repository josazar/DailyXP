...
void main(){
  ...
  for(int n=0;n<AA;n++)
  {
    ...
    vec2 p=(4.*(gl_FragCoord.xy+o)-u_resolution.xy)/u_resolution.x;
    #else
    ...
    float u=ceil(w);
    float f=fract(w);
    vec3 col=vec3(.851,.6039,.8941)+.3*sin(3.*u+vec3(0.,1.8,2.));
    
    // filtered drop-shadow
    float sha=smoothstep(0.,.05,f);
    ...
    // lighting
    // col*=.3+.7*sha;
    col*=.8+.2*vec3(1.,.9,.3)*dot(nor,vec3(.2275,.1569,.4588));
